...
const Header = () => {
  ...
  const handleFocus = () => {
    setIsFocus(true);
    setShowMenu(false); // langsung sembunyikan menu
  };

  const handleBlur = () => {
    setIsFocus(false);
    // delay supaya tunggu transisi input selesai dulu
    setTimeout(() => {
      setShowMenu(true);
    }, 300); // sama dengan duration-300 di Tailwind
  };
  ...
  return (
    ...
    >
      ...
      <div className="hidden xl:flex items-center md:px-20 bg-amber-50 py-3">
        ...
        <div
          className={`flex  ${
            showMenu ? "justify-end" : "justify-end"
          } items-center gap-[10%] w-[80vw] px-2 ml-12 `}
        >
          <AnimatePresence>
            <motion.ul
              initial={{ opacity: 1 }} // posisi awal (transparan + agak ke atas)
              animate={{ opacity: showMenu ? 1 : 0 }} // animasi masuk
              exit={{ opacity: 0, y: 0, x: 0 }} // animasi keluar
              transition={{ duration: 0.3, ease: "easeInOut" }}
              className={`${
                showMenu ? "flex" : "hidden"
              } items-center gap-x-6 h-full`}
            >
              {routes.map((route) => (
                <li
                  key={route.href}
                ...
                >
                  ...
                  {route.isOpen && (
                    ...
                    >
                      {(route.label === "Concept" || route.label === "Konsep") && (
                        <div>
                          <div className="flex justify-center gap-8">
                            {conceptData?.map((item, index) => (
                              <Link
                                ...
                                key={index}
                                className="bg-amber-50 py-6 px-16 text-center rounded-2xl shadow-md group"
                              >
                                <Image
                                  alt={item.icon.url}
                                  ...
                                  height={100}
                                  width={100}
                                  className="mb-4 group-hover:scale-110 duration-300  transition-transform"
                                />
                                <p
                      ...
                      {(route.label === "Developments" || route.label === "Development") && (
          ...
          <div className={`flex items-center justify-end `}>
            <div className="flex items-center justify-center">
              <form onSubmit={handleSubmitQueryInput}>
                <input
                  ref={inputRef}
                  onFocus={handleFocus}
                  onBlur={handleBlur}
                  ...
                  className="
                  w-40
                  focus:w-[800px]   /* langsung full, tanpa prefix lg */
                  transition-all
                  duration-300
                  ease-in-out
                  px-3 py-1
