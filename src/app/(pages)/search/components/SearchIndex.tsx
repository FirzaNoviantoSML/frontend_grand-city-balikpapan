...
const SearchIndex = () => {
  ...
  const handleBtnSearch = (e: FormEvent) => {
    e.preventDefault();
    handleSearch(e);
  }
    ...
    <div className="bg-[#FFFCE4]">
      <div className="flex justify-center  mt-16">
        <div className=" flex justify-start flex-col min-h-[80vh] w-[80%] gap-12 my-12">
          <div className="border-b-1 border-neutral-200 pb-6">
            <form className="col-span-4 flex" onSubmit={handleBtnSearch}>
                  <input
                    value={query}
                ...
                <button
                  type="submit"
                  className="bg-amber-600 text-white px-4 py-1 rounded-r-md cursor-pointer"
                >
                  <IoIosSearch />
                </button>
            </form>
            ...
            {dataSearch&& dataSearch?.map((item, index) => {
              return (
                ...
                key={index} className="grid grid-cols-5 gap-4 px-2 border-b-1 border-neutral-200 py-2">
                  <div className="relative h-[130px] col-span-2 md:col-span-1">
                    <Image
                  ...
                  <div className="col-span-3 md:col-span-4 flex justify-start flex-col">
                    <p className="text-md md:text-2xl text-amber-600 font-medium">
                      {item.title}
                    </p>
                    <p className="text-sm text-neutral-400">{item.type}</p>
                    <p className="text-sm md:text-md line-clamp-3">{item.description}</p>
                  </div>
                </Link>
              );
            })}
