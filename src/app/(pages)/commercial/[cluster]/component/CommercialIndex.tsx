...
import CommercialOthers from "./CommercialOthers";
...
const CommercialIndex = ({ slug }: { slug: string }) => {
  const isMobile = useIsMobile();
  const { language } = useLanguage();
  const { clusterDetailData, isLoading } = useGetClusterDetail(language, slug);
  const {UnitTypeData,isLoading: isLoadingUnitType} = useGetUnitTypeList(language,slug)
...
>
    ...
    <span className={`px-3 py-1 rounded-full ${item.available? "bg-[#86B81B]":"bg-black/60"} text-white text-xs md:text-sm backdrop-blur-sm`}>
...
<div className="my-12 ">
    <CommercialGallery gallery={clusterDetailData?.gallery} language={language} title={clusterDetailData?.title} />
</div>
<div>
  <CommercialOthers slug={slug}/>
</div>
